...
func (store *UserStore) New(ctx context.Context) authboss.User {
	return &User{}
}

func (store *UserStore) Create(ctx context.Context, user authboss.User) error {
	store.db.Create(user).Scan(&user)
	return store.db.Error
}

func (store *UserStore) Load(ctx context.Context, key string) (authboss.User, error) {
	user := User{}
	store.db.Where("email = ?", key).First(&user)
	err := store.db.Error
	return &user, err
}

func (store *UserStore) Save(ctx context.Context, user authboss.User) (err error) {
	_, err = store.Load(ctx, user.GetPID())
	store.db.Model(&user).Updates(user)
	err = store.db.Error
	return
}

func (store *UserStore) NewFromOAuth2(ctx context.Context, provider string, details map[string]string) (user authboss.OAuth2User, err error) {
	return
}

func (store *UserStore) LoadByConfirmSelector(ctx context.Context, selector string) (user authboss.ConfirmableUser, err error) {
	return
}

func (store *UserStore) LoadByRecoverSelector(ctx context.Context, selector string) (user authboss.RecoverableUser, err error) {
	return
}

func (store *UserStore) AddRememberToken(ctx context.Context, pid, token string) (err error) {
	return
}

func (store *UserStore) DelRememberTokens(ctx context.Context, pid string) (err error) {
	return
}

func (store *UserStore) UseRememberToken(ctx context.Context, pid, token string) (err error) {
	return
}
